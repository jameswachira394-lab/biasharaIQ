...
class MpesaService:
    ...
    def get_access_token(self) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                "M-Pesa authentication request timed out (network issue or API slow)")
            return None
    ...
    def initiate_stk_push(self, phone: str, amount: int,
                          account_ref: str) -> Dict[str, Any]:
        """Initiate M-Pesa STK Push (Lipa na M-Pesa Online)."""
        # Validate callback URL
        if not self.callback_url:
            return {
                "error": "M-Pesa callback URL not configured. Set MPESA_CALLBACK_URL in .env",
                "status": 500}

        access_token = self.get_access_token()
        if not access_token:
            return {
                "error": "Failed to authenticate with M-Pesa. Check your credentials in .env",
                "status": 500}
        ...
        try:
            ...
            logger.debug(
                f"M-Pesa API Response: {response.status_code} - {response.text}")
            ...
            if response.status_code != 200:
                error_msg = data.get(
                    "errorMessage", data.get(
                        "error", "Unknown error from M-Pesa API"))
